...
def slope_original_signal(data):
    ...
    sclr = PowerTransformer()
    sclr.fit(train_features)
    train_features = sclr.transform(train_features)
    sclr_filename = 'sclr_POW_SLP-ORS.mdl'
    pickle.dump(sclr, open(sclr_filename, 'wb'))
...
def slope_original_signal_val_last(data):
    ...
    sclr = PowerTransformer()
    sclr.fit_transform(all_features)
