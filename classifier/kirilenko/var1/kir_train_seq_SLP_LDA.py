...
def read_dat_by_clapans(data, clapan_length=5000, prestimul_length=0, stimul_delay=0):
    ...
    clapans_obj_list = [Clapan(clapans_values[i], clapans_indexes[i], clapan_length, prestimul_length, stimul_delay) for
                        i in range(clapans_values.shape[0])]
...
def multichannel_bandpass_filter(input_data, Fd=1000, lowcut=60, highcut=120, order=2):
    for i in range(input_data.shape[1]):
        input_data[:, i] = butterworth_banpass_filter(data=input_data[:, i], lowcut=lowcut, highcut=highcut, Fd=Fd,
                                                      order=order)
    return input_data
...
def difference_2d_stimul_features(input_data, input_clapan, feature_type='Slope_original_signal', normirovka=True,
                                  Fd=1000):
    """
...
def slope_original_signal(data):
    ...
    for clapan in tqdm(train_clapans):
        features = difference_2d_stimul_features(input_data=train_data[:, :-2], input_clapan=clapan,
                                                 feature_type='Slope_original_signal', normirovka=False, Fd=Fd)
    ...
    usd_train_samples = np.sort(
        np.concatenate([np.where(raw_train_dataset[:, -1] == usd_class_it)[0] for usd_class_it in train_classes]))
    ...
    for train_group_it in range(len(mixture_groups_2)):
        ...
        idxs_of_train_group = np.sort(
            np.concatenate([np.where(raw_train_dataset[:, -1] == clapan_it)[0] for clapan_it in train_group]))
...
def train(filename):
    ...
    data, clapans_obj_list = read_dat_by_clapans(data_matrx, clapan_length=clapan_length,
                                                 prestimul_length=prestimul_length, stimul_delay=stimul_delay)
