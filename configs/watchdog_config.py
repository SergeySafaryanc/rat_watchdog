...
num_channels = 8  # число каналов ЭГ+дыхание+метка

count_train_stimuls = 80  # число тренировок
train_step = 20  # число смещений
...
odors = [("Табак", "#ffff00"), ("Жидкий дым", "#ffff00"), ("Метилбензоат", "#ffff00"), ("ТНТ", "#ffff00"),
         ("5", "#ffff00"), ("3", "#ffff00")]  # Расшифровка и цвет вещества (на экран и в таблицу)

odors_set = [1, 2, 4, 8]  # метки веществ (числовые)

unite = [[0], [1], [2], [3]]  # передаётся в классификатор для обучения (теста после mixture_groups)

unite_test = [[0], [1], [2], [3]]  # Для подсчета результатов при смене клапанов
...
mixture_groups = [[1], [2], [4], [8]]  # для классификации по клапанам (для Колиного)
