...
inp_path = "/home/sergey/PycharmProjects/rat_watchdog/data/inp"
out_path = "/home/sergey/PycharmProjects/rat_watchdog/data/out"

sampling_rate = 1000  # частота дискретизации
decimate_rate = 10  # прорежение данных
num_channels = 18  # число каналов ЭГ+дыхание+метка

num_counter_for_refresh_animal = 195  # количество
...
odors = [("Мухамад 2", "#ffff00"), ("Гребеньков 2", "#ffff00"), ("Лежнев 2", "#ffff00"),
         # ("Вертилов 2", "#ffff00"),
         # ("Губа 2", "#ffff00"), ("Чуб 2", "#ffff00"),
         ("5", "#ffff00"), ("3", "#ffff00")]  # Расшифровка и цвет вещества (на экран и в таблицу)тки веществ (числовые)
odors_set = [8, 16, 32]  # метки веществ (числовые)
# odors_set = [1,2,4,8,16,32]  # метки веществ (числовые)

unite = [[0], [1], [4]]  # передаётся в классификатор для обучения (теста после mixture_groups)

unite_test = [[3], [2], [5]]  # Для подсчета результатов при смене клапанов

# для конвертации приходящих меток
# labels_map = {1: 4, 2: 2, 16: 6} # for test
labels_map = {8: 2, 16: 3, 32: 5}
# labels_map = {1: 0, 2: 1, 4: 4}
...
mixture_groups = [[8], [16], [32]]  # для классификации по клапанам (для Колиного)
# mixture_groups = [[1, 4], [16, 8, 1]]          #для группировки. Пусть [4,5- Иное], [1,3- НС], [2,6 - ВВ]. Для Иное метка-'0', для НС метка-'1', для ВВ метка-'2'
